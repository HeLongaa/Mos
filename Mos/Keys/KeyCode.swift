...
struct KeyCode {
    ...
    /// 数字键盘相关键 (方向键 + 导航键)
    /// macOS 自动为这些键携带 maskNumericPad 和 maskSecondaryFn 标志
    /// 合成事件也需要补充这些标志, 否则系统快捷键 (如 Ctrl+↑ 调度中心) 不会触发
    static let numpadAwareKeys: Set<UInt16> = [
        // 方向键
        126, 125, 123, 124,
    ...
    static let mouseMap: [UInt16: String] = [
        ...
        19: "🖱19", 20: "🖱20",
        // 虚拟滚轮倾斜码 (非真实硬件按键, 用于 Logi 等鼠标的横向拨动)
        21: "🖱←", 22: "🖱→",
        // 虚拟滚轮上下码 (普通滚轮垂直方向, 用于组合触发器)
        23: "🖱↑", 24: "🖱↓"
    ]
    static let mouseMainKeys: [UInt16] = [0,1]  // Only protect left/right clicks, allow middle button without modifiers

    /// 虚拟滚轮倾斜码 (Logi 等鼠标横向拨动)
    static let scrollWheelLeft: UInt16 = 21
    static let scrollWheelRight: UInt16 = 22
    /// 虚拟滚轮上下码 (普通滚轮垂直滚动, 用于组合触发器)
    static let scrollWheelUp: UInt16 = 23
    static let scrollWheelDown: UInt16 = 24
}
